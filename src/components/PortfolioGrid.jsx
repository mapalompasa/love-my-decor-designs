    ...
    <section id="portfolio" className="py-16 md:py-28 border-t border-white/10 relative scroll-mt-20">
      <div id="collections" className="max-w-[1720px] mx-auto px-4 sm:px-6 lg:px-8 scroll-mt-24">
        ...
        <div className="mb-10 md:mb-16">
          ...
          <div className="flex items-center gap-2 sm:gap-4 border-b border-white/10 pb-4 sm:pb-6 overflow-x-auto no-scrollbar sm:flex-wrap -mx-4 px-4 sm:mx-0 sm:px-0">
            {categories.map((cat) => (
              <button
                key={cat.label}
                onClick={() => setActiveCategory(cat.label)}
                className={`flex-shrink-0 sm:flex-shrink px-4 py-2.5 text-xs font-[900] uppercase tracking-tight transition-all flex items-center gap-2 border min-h-[42px] sm:min-h-0 ${
        ...
        <div className="grid grid-cols-8 md:grid-cols-16 gap-6 md:gap-8">
          {filteredProjects.map((project, idx) => {
            const desktopSpan = project.colSpan.includes('md:') ? `lg:${project.colSpan.split('md:')[1]}` : 'lg:col-span-6';
            ...
            return (
              ...
              >
                ...
                <div className={`relative ${project.aspect} w-full overflow-hidden bg-[#141414] border border-white/10 mb-4`}>
                  <img
                    src={project.image}
                    alt={project.title}
                  ...
                  {/* Quick inspect prompt (always visible on touch/mobile, hover on desktop) */}
                  <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between opacity-90 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300">
        ...
        <div className="mt-16 md:mt-24 p-8 bg-[#120d14] border border-[#2c1930] flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h4 className="text-xl sm:text-2xl font-[900] uppercase tracking-tight text-white mb-2">
          ...
          <a
            href="#booking"
            className="flex-shrink-0 px-6 py-3.5 bg-[#83338a] text-white font-[900] text-xs uppercase tracking-tight hover:bg-[#993da3] transition-all flex items-center gap-2 border border-[#c966d4]/40 shadow-lg shadow-[#83338a]/25"
          >
